...
function parseTSDoc(foundComment: IFoundComment): void {
	...
	if (parserContext.docComment.modifierTagSet.hasTag(customModifierDefinition)) {
		console.log(
			os.EOL + colors.cyan(`The ${customModifierDefinition.tagName} modifier was FOUND.`),
		);
	} else {
		console.log(
			os.EOL + colors.cyan(`The ${customModifierDefinition.tagName} modifier was NOT FOUND.`),
		);
	}
...
export function advancedDemo(): void {
	...
	const inputFilename: string = path.resolve(
		path.join(__dirname, '..', 'input', 'advanced-input.ts'),
	);
