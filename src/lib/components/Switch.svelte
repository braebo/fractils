...
- `title` *string* - Visible to screen readers / tooltips.
- `on` *string* - The content to display when the switch is on.
- `off` *string* - The content to display when the switch is off.

## Events
- `on:click` - Toggle the switch.
...
.slider {
	...
	box-shadow: -1px 1px 0.3rem rgba(0, 0, 0, 0.5) inset, 0px 1px 0.1rem rgba(0, 0, 0, 0.5) inset;
...
#{$thumb} {
	...
	box-shadow: 0.3px 1px 0 rgba(var(--bg-d-rgb, rgb(1, 1, 1)), 0.5),
...
.off {
	...
	transition: opacity 0.2s ease-out, transform var(--transition);
}
