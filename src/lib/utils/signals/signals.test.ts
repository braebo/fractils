import { untracked, computed, Signal, signal, effect, batch } from './signals'
...
describe('signal', () => {
	...
	describe('.peek()', () => {
		...
		describe('computed behavior', () => {
			it('should not make surrounding computed depend on the signal', () => {
				...
				const d = computed(spyFunction)

				d.value // Access to trigger computation
				expect(spyFunction).toHaveBeenCalledTimes(1)

				s.value = 2
				d.value // Access again to see if recomputation happens
				expect(spyFunction).toHaveBeenCalledTimes(1)
			})
		})
	})
	...
	it('signals should be identified with a symbol', () => {
		const a = signal(0)
		expect(a.brand).to.equal(Symbol.for('fractils-signals'))
	})
...
describe('computed()', () => {
	...
	describe('garbage collection', () => {
		let shouldSkip = typeof gc === 'undefined'

		beforeAll(() => {
			if (shouldSkip) {
				console.warn('Skipping GC tests: `gc` not defined')
			}
		})
	...
	describe('graph updates', () => {
		...
		it('should only update every signal once (jagged diamond graph + tails)', () => {
			...
			// top to bottom
			expect(eSpy).not.toHaveBeenCalledWith(fSpy)
			// left to right
			expect(fSpy).not.toHaveBeenCalledWith(gSpy)
		})
