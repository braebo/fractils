import { SPACE_MONO, SPACE_16_COLORS, SPACE_256_COLORS, SPACE_TRUE_COLORS } from './color-spaces'
 ...
 */
const detectColorSpace = (env: object, isTTY: boolean): number => {
	const inEnvSome = (arr: string[]) => arr.some(val => val in env)
	// @ts-ignore
	...
	if (
		/^screen|^tmux|^xterm|^vt[1-5][0-9]([0-9])?|^ansi|color|cygwin|linux|mintty|rxvt/i.test(
			term,
		)
	)
		return SPACE_16_COLORS
...
export const getColorSpace = (mockThis?: typeof globalThis): number => {
	/**
	 * Detect whether flags exist with `-` or `--` prefix in command-line arguments.
	 *
	 * @param regex - The RegEx to match all possible flags.
	 */
	const hasFlag = (regex: RegExp): boolean => !!argv.find((value: string) => regex.test(value))
	...
	const isForceEnabled =
		(FORCE_COLOR in env && forceColor) || hasFlag(/^-{1,2}color=?(true|always)?$/)
