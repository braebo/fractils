...
export async function extract(verbose = false) {
	...
	commentCount += comments.reduce(
		(acc, { files }) => acc + files.reduce((acc, { variables, types }) => acc + variables.length + types.length, 0),
		0,
	)
	...
	l(dim('.doc.json written       '), g(jsonCount))
	n()
...
async function writeComments(comments: ParsedCategory[], verbose = false) {
	...
	for (const { files } of comments) {
		for (const parsedFile of files) {
			...
			await writeFile(out, JSON.stringify(parsedFile, null, 2), {
				encoding: 'utf-8',
			})
