	...
	if (statements) {
		for (const statement of statements) {
			...
			if (
				...
				// @ts-expect-error no idea why it's complaining here
				...
				const snippet =
					...
					// 	})
					// )
					snippet_unformatted.replace(/\s*(\/\*…\*\/)\s*/g, '/*…*/').trim()
...
function munge_type_element(member: ts.TypeElement, depth = 1): Extracted | undefined {
	...
	if (
		...
		for (const child of member.type.members) {
			const extracted = munge_type_element(child, depth + 1)
...
async function read_d_ts_file(file: string) {
